...
import { sendTelegramMessage, formatSubsDeadlineMessage, formatListingMessage } from '../src/services/telegram';
import { dbService } from '../src/services/db';

export default async function handler(req: any, res: any) {
  try {
    const { type, token, chat_id } = req.query;
    console.log(`[Vercel Cron] Triggered cron check. Type: ${type || 'ALL'}`);

    const today = new Date().toISOString().slice(0, 10);
    const ipos = await fetchAllIpoSchedules(true);

    // 사용자 설정 또는 환경변수에서 텔레그램 설정 로드
    const prefs = dbService.getPreferences();
    const botToken = (token as string) || process.env.TELEGRAM_BOT_TOKEN || prefs.telegramBotToken || '';
    const chatId = (chat_id as string) || process.env.TELEGRAM_CHAT_ID || prefs.telegramChatId || '';

    const telegramConfig = { botToken, chatId };
    ...
    if (type === 'listing' || !type) {
      ...
      for (const ipo of listingIpos) {
        if (isMatched(ipo.underwriters)) {
          const msg = formatListingMessage(ipo);
          if (botToken && chatId) {
            await sendTelegramMessage(telegramConfig, msg);
          }
